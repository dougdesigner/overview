...
function Navigation() {
  ...
  const { isDemoMode, accounts, holdings, hasViewedStocksPage } =
    usePortfolioStore()
  ...
  const getOnboardingDot = (path: string) => {
    ...
    if (accounts.length > 0 && holdings.length === 0 && path === "/holdings")
      return true
    // Has holdings, not viewed stocks → dot on Stocks tab
    if (holdings.length > 0 && !hasViewedStocksPage && path === "/exposure")
      return true
    return false
  }
  ...
  return (
    <>
      <div className="shadow-s z-20 bg-white dark:bg-gray-950 sm:sticky sm:top-0">
      ...
      <div className="pb-safe fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 sm:hidden">
        <div className="flex items-center justify-around px-2 py-2">
          ...
          <Dialog.Root open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            {/* Bottom overlay - gradient fades content behind drawer down to nav */}
            {isAddOpen && (
              <div className="pointer-events-none fixed inset-x-0 bottom-20 z-[45] h-52 bg-gradient-to-b from-white/0 via-white/90 to-white dark:from-gray-950/0 dark:via-gray-950/90 dark:to-gray-950" />
            )}
