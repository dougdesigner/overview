...
/**
 * Script to fetch and cache company overview data for all VTI holdings
...
const VTI_FILE = path.join(process.cwd(), "src", "data", "etf-profiles", "VTI.json")
...
interface VTIHolding {
  symbol: string
  weight: number
}

interface VTIProfile {
  symbol: string
  name: string
  holdings: VTIHolding[]
}
...
async function main() {
  console.log("🚀 Starting VTI company data caching process...")
  console.log()

  // Read VTI holdings
  const vtiData = JSON.parse(await fs.readFile(VTI_FILE, "utf-8")) as VTIProfile

  // Extract unique stock symbols (filter out n/a)
  const allSymbols = vtiData.holdings
    .map(h => h.symbol)
    .filter(s => s !== "n/a") // Filter out non-stocks

  console.log(`📊 Found ${allSymbols.length} stock symbols in VTI`)
  ...
  console.log("📈 SUMMARY")
  console.log("=".repeat(60))
  console.log(`Total symbols in VTI: ${allSymbols.length}`)
  ...
  if (totalErrors > 0) {
    ...
    console.log("🎉 All VTI holdings successfully cached!")
  }

  console.log()
  console.log("💡 Next steps:")
  console.log("   1. Check the treemap visualization for proper sector groupings")
  console.log("   2. Run this script for other ETFs if needed")
}
